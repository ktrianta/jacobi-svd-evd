...
size_t evd_cyclic_oneloop_vectorize(struct matrix_t Data_matr, struct matrix_t Data_matr_copy,
        ...
        for (size_t p = 0; p < n; p++) {
            for (size_t q = p + 1; q < n; q++) {
                ...
                for (i = 0; i < n - step + 1; i += step) {
                    ...
                    __m256d A_piv = _mm256_load_pd(A + n * p + i);
                    __m256d A_qiv = _mm256_load_pd(A + n * q + i);
                    ...
                    if (!(i + step - 1 >= p && i <= p) && !(i + step - 1 >= q && i <= q)) {
                        A[n * i + p] = nA_piv[0];
                        A[n * (i + 1) + p] = nA_piv[1];
                        A[n * (i + 2) + p] = nA_piv[2];
                        A[n * (i + 3) + p] = nA_piv[3];
...
size_t evd_cyclic_oneloop_vectorize_old(struct matrix_t Data_matr, struct matrix_t Data_matr_copy,
                                        struct matrix_t Eigen_vectors, struct vector_t Eigen_values, int epoch) {
    assert(Data_matr.rows == Data_matr.cols);
    double* A = Data_matr_copy.ptr;
    double* V = Eigen_vectors.ptr;
