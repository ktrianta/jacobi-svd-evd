def gen_subprocedure_vectorized(unroll_cnt):
    VEC_SIZE = 4
...
################# FIRST LOOP #################################
    ...
    line_list.append('    double b_ik = B[n * i + k];')
    line_list.append('    double b_jk = B[n * j + k];')
...
################# SECOND LOOP #################################
    ...
    line_list.append('}')
    line_list.append('for (; k < n; ++k) {')
    line_list.append('    double b_ki = B[n * k + i];')
    line_list.append('    double b_kj = B[n * k + j];')
...
################# THIRD LOOP #################################
    ...
    line_list.append(f'for (; k + {unroll_elems-1} < n; k += {unroll_elems}) {{')
    for u in range(unroll_cnt):
        line_list.append(f'    vi_{u} = _mm256_set_pd(U[n * (k + {u*VEC_SIZE}) + i]')
        for m in range(1, VEC_SIZE):
            line_list[-1] += f', U[n * (k + {u*VEC_SIZE + m}) + i]'
        line_list[-1] += ');'
    for u in range(unroll_cnt):
        line_list.append(f'    vj_{u} = _mm256_set_pd(U[n * (k + {u*VEC_SIZE}) + j]')
        for m in range(1, VEC_SIZE):
            line_list[-1] += f', U[n * (k + {u*VEC_SIZE + m}) + j]'
        line_list[-1] += ');'
    line_list.append('')
    for u in range(unroll_cnt):
        line_list.append(f'    left_{u} = _mm256_mul_pd(v_s, vj_{u});')
    ...
    for u in range(unroll_cnt):
        for m in range(VEC_SIZE):
            line_list.append(f'    U[n * (k + {u*VEC_SIZE+m}) + i] = left_{u}_ptr[{VEC_SIZE-1-m}];')
    for u in range(unroll_cnt):
        for m in range(VEC_SIZE):
            line_list.append(f'    U[n * (k + {u*VEC_SIZE+m}) + j] = right_{u}_ptr[{VEC_SIZE-1-m}];')
    line_list.append('}')
    line_list.append('for (; k < n; ++k) {')
    line_list.append('    double u_ki = U[n * k + i];')
    line_list.append('    double u_kj = U[n * k + j];')
    line_list.append('    double left = cf.c1 * u_ki - cf.s1 * u_kj;')
    line_list.append('    double right = s1k * u_ki + c1k * u_kj;')
    line_list.append('    U[n * k + i] = left;')
    line_list.append('    U[n * k + j] = right;')
    line_list.append('}')
    line_list.append('')
...
################# FOURTH LOOP #################################
    ...
    line_list.append(f'for (; k + {unroll_elems-1} < n; k += {unroll_elems}) {{')
    for u in range(unroll_cnt):
        line_list.append(f'    vi_{u} = _mm256_set_pd(V[n * (k + {u*VEC_SIZE}) + i]')
        for m in range(1, VEC_SIZE):
            line_list[-1] += f', V[n * (k + {u*VEC_SIZE + m}) + i]'
        line_list[-1] += ');'
    for u in range(unroll_cnt):
        line_list.append(f'    vj_{u} = _mm256_set_pd(V[n * (k + {u*VEC_SIZE}) + j]')
        for m in range(1, VEC_SIZE):
            line_list[-1] += f', V[n * (k + {u*VEC_SIZE + m}) + j]'
        line_list[-1] += ');'
    line_list.append('')
    for u in range(unroll_cnt):
        line_list.append(f'    left_{u} = _mm256_mul_pd(v_s, vj_{u});')
    ...
    for u in range(unroll_cnt):
        for m in range(VEC_SIZE):
            line_list.append(f'    V[n * (k + {u*VEC_SIZE+m}) + i] = left_{u}_ptr[{VEC_SIZE-1-m}];')
    for u in range(unroll_cnt):
        for m in range(VEC_SIZE):
            line_list.append(f'    V[n * (k + {u*VEC_SIZE+m}) + j] = right_{u}_ptr[{VEC_SIZE-1-m}];')
    line_list.append('}')
    line_list.append('for (; k < n; ++k) {')
    line_list.append('    double v_ki = V[n * k + i];')
    line_list.append('    double v_kj = V[n * k + j];')
    line_list.append('    double left = cf.c2 * v_ki - cf.s2 * v_kj;')
    line_list.append('    double right = cf.s2 * v_ki + cf.c2 * v_kj;')
    line_list.append('    V[n * k + i] = left;')
    line_list.append('    V[n * k + j] = right;')
    line_list.append('}')
